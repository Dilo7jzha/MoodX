...
onMounted(() => {
    ...
    geocoder.value = new MapboxGeocoder({
        ...
        mapboxgl: mapboxgl,
        placeholder: 'Search for mental health centers',
        marker: false,
    });
    ...
    const destinationPopup = new mapboxgl.Popup()
        .setHTML('<h3>MoodX Mental Health Organization</h3><p>Providing support and resources.</p>')
        .on('open', () => {
            // Remove aria-hidden from the close button when the popup opens
            document.querySelectorAll('.mapboxgl-popup-close-button').forEach(button => {
                button.removeAttribute('aria-hidden');
            });
        });
